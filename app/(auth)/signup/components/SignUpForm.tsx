...
export const SignUpForm = () => {
    ...
    const onSubmitReg = async (data: any) => {
        ...
        const regRes = await fetch('http://92.51.47.29:81/auth/register',
            {
                method: 'POST',
                headers: {
        ...
        if(regRes.status != 200) {
                setError("password", {
                    type: "custom",
                    message: regRes.json()['exception'] ? 'Ошибка' : "Incorrect credentials",
                });
    ...
    return (
        <div className={cx(montserratAlternates.className, styles.formWrapper)}>
            ...
            <div className={cx(styles.text, styles.passTitle)}>Почта</div>
            <Controller
                name="email"
                control={control}
                rules={{
